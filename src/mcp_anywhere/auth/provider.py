...
class MCPAnywhereAuthProvider(OAuthAuthorizationServerProvider):
    ...
    async def exchange_authorization_code(
        ...
        if (
            ...
            or auth_code_data["redirect_uri"] != str(client.redirect_uris[0])
        ):
            raise TokenError("invalid_grant")
    ...
    async def exchange_refresh_token(self, client: OAuthClientInformationFull, refresh_token: RefreshToken) -> AccessToken:
