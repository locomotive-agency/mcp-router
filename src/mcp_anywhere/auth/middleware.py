...
class JWTAuthMiddleware(BaseHTTPMiddleware):
    ...
    def _create_auth_error_response(self, error: str, description: str = None, status_code: int = 401) -> JSONResponse:
        """
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        logger.debug(f"Authenticated request for user: {token_payload.get('username')} on path: {path}")
