...
async def homepage(request: Request) -> HTMLResponse:
    ...
    try:
        async with get_async_session() as db_session:
            ...
            stmt = select(MCPServer).options(selectinload(MCPServer.tools)).where(MCPServer.is_active == True).order_by(MCPServer.name)
            result = await db_session.execute(stmt)
            servers = result.scalars().all()
            
        return templates.TemplateResponse(request, "index.html", get_template_context(request, servers=servers))
    
    except Exception as e:
        # Log error and show empty server list
        logger.error(f"Error loading servers: {e}")
        return templates.TemplateResponse(request, "index.html", get_template_context(request, servers=[], error=str(e)))
...
async def server_detail(request: Request) -> HTMLResponse:
    ...
    try:
        async with get_async_session() as db_session:
            ...
            tools_stmt = select(MCPServerTool).where(MCPServerTool.server_id == server_id).order_by(MCPServerTool.tool_name)
...
async def edit_server_get(request: Request) -> HTMLResponse:
    ...
    try:
        ...
        return templates.TemplateResponse(request, "servers/edit.html", get_template_context(request, server=server))
        ...
        try:
            ...
            try:
                ...
                if request.headers.get("HX-Request"):
                    return templates.TemplateResponse(
                        ...
                        get_template_context(request, github_url=analyze_data.github_url, analysis=analysis)
                    )
                else:
                    return templates.TemplateResponse(
                        request, 
                ...
                if request.headers.get("HX-Request"):
                    return templates.TemplateResponse(
                        request, 
                        "partials/analysis_result.html", 
                        get_template_context(request, github_url=analyze_data.github_url, analysis=analysis, warning="Repository analysis failed. Please fill out the form manually.")
                    )
                else:
                    return templates.TemplateResponse(
                        request, 
                ...
                if request.headers.get("HX-Request"):
                    return templates.TemplateResponse(
                        request, 
                        "partials/analysis_result.html", 
                        get_template_context(request, github_url=analyze_data.github_url, error=error_msg)
                    )
                else:
                    return templates.TemplateResponse(
                        request, 
            ...
            if request.headers.get("HX-Request"):
                return templates.TemplateResponse(
                    request, 
                    "partials/analysis_result.html", 
                    get_template_context(request, github_url=form_data.get("github_url", ""), errors=errors)
                )
            else:
                return templates.TemplateResponse(
                    request, 
            ...
            if request.headers.get("HX-Request"):
                return templates.TemplateResponse(
                    request, 
                    "partials/analysis_result.html", 
                    get_template_context(request, github_url=form_data.get("github_url", ""), error=error_msg)
                )
            else:
                return templates.TemplateResponse(
                    request, 
    ...
    # Handle save button
        ...
        try:
            ...
            return templates.TemplateResponse(
                request, 
                "servers/add.html", 
                get_template_context(request, form_data=form_data, error="A server with this name already exists.")
            )
            ...
            return templates.TemplateResponse(
                request, 
                "servers/add.html", 
                get_template_context(request, form_data=form_data, error="Error saving server. Please try again.")
            )
    
    # Invalid form submission
    return templates.TemplateResponse(request, "servers/add.html", get_template_context(request, form_data=form_data))
...
routes = [
    ...
    Route("/servers/{server_id}/tools/{tool_id}/toggle", endpoint=toggle_tool, methods=["POST", "GET"]),
