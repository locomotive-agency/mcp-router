...
class FastMCPLifespanWrapper:
    ...
    def __init__(self, fastmcp_app: ASGIApp):
    ...
    async def _ensure_lifespan_started(self):
        ...
        async def lifespan_send(message):
            if message["type"] == "lifespan.startup.complete":
                ...
                logger.error(f"FastMCP lifespan startup failed: {message.get('message', 'Unknown error')}")
                raise RuntimeError("FastMCP lifespan startup failed")
            elif message["type"] == "lifespan.shutdown.complete":
                logger.info("FastMCP lifespan shutdown complete")
            elif message["type"] == "lifespan.shutdown.failed":
                logger.error(f"FastMCP lifespan shutdown failed: {message.get('message', 'Unknown error')}")
