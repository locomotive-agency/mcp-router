...
TOKEN_EXPIRY = int(os.environ.get('OAUTH_TOKEN_EXPIRY', '3600'))  # 1 hour default
...
def create_oauth_blueprint():
    ...
    def authorize():
        ...
        if code_challenge_method != 'S256':
            return jsonify({"error": "invalid_request", "error_description": "Only S256 code challenge method supported"}), 400
        
        if not code_challenge:
            return jsonify({"error": "invalid_request", "error_description": "PKCE code_challenge required"}), 400
...
def create_bearer_auth_provider():
    ...
    async def validate_bearer_token(headers: Dict[str, str]) -> Optional[Dict]:
