...
from wtforms import StringField, SelectField, TextAreaField, FieldList, FormField, BooleanField
...
class ServerForm(FlaskForm):
    ...
    runtime_type = SelectField('Runtime Type',
                             choices=[
                                 ('npx', 'Node.js (npx)'),
                                 ('uvx', 'Python (uvx)'),
                                 ('docker', 'Docker')
    ...
    install_command = StringField('Install Command',
                                validators=[Length(max=500)],
                                render_kw={"placeholder": "npm install or pip install -e ."})
    
    start_command = StringField('Start Command',
