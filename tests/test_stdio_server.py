...
async def test_run_stdio_server_creates_app_and_tasks():
    ...
    with (
        patch(
            "mcp_anywhere.transport.stdio_server.create_app", return_value=mock_app
        ) as mock_create_app,
        patch("mcp_anywhere.transport.stdio_server.uvicorn.Config") as mock_config,
        patch(
            "mcp_anywhere.transport.stdio_server.uvicorn.Server", return_value=mock_server
        ...
        mock_config.assert_called_once_with(mock_app, host="127.0.0.1", port=8001, log_level="info")
...
async def test_run_stdio_server_uses_correct_defaults():
    ...
    with (
        patch("mcp_anywhere.transport.stdio_server.create_app", return_value=mock_app),
        patch("mcp_anywhere.transport.stdio_server.uvicorn.Config") as mock_config,
        ...
        mock_config.assert_called_once_with(mock_app, host="0.0.0.0", port=8000, log_level="info")
...
async def test_run_stdio_server_task_cancellation():
    ...
    with (
        ...
        patch("asyncio.create_task") as mock_create_task,
        patch(
            "asyncio.gather", new_callable=AsyncMock, side_effect=asyncio.CancelledError
        ) as mock_gather,
    ):
