...
class TestOAuthIntegrationFlow:
    ...
    def setup_method(self):
        ...
        async def mock_session_factory():
            ...
            def mock_scalar_side_effect(stmt):
                ...
                if hasattr(stmt, 'compile') and 'oauth2_clients' in str(stmt.compile(compile_kwargs={"literal_binds": True})):
                    return db_client
                return test_user
    ...
    def test_csrf_protection_integration(self):
        ...
        assert self.csrf_protection.validate_state(state, "test_client", "http://localhost:3000/callback")
        
        # Verify state is consumed (one-time use)
        assert not self.csrf_protection.validate_state(state, "test_client", "http://localhost:3000/callback")
