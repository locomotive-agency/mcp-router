...
async def test_tool_filter_middleware_passes_non_mcp_requests():
    ...
    app = Starlette(
        routes=[Route("/test", dummy_endpoint, methods=["GET"])],
...
async def test_tool_filter_middleware_filters_disabled_tools():
    ...
    mock_disabled_tools = [Mock(tool_name="disabled_tool", is_enabled=False)]
    ...
    app = Starlette(
        routes=[Route("/mcp/tools/list", mock_mcp_endpoint, methods=["POST"])],
        middleware=[Middleware(ToolFilterMiddleware)],
    )

    with (
        patch("mcp_anywhere.core.middleware.get_async_session") as mock_session,
