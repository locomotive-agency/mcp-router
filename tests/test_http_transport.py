...
async def test_run_http_server_creates_uvicorn_server():
    ...
    with patch('mcp_anywhere.transport.http_server.uvicorn') as mock_uvicorn, \
        ...
        mock_uvicorn.Config.assert_called_once_with(mock_app, host="0.0.0.0", port=8000, log_level="info")
...
async def test_run_http_server_with_custom_host_port():
    ...
    with patch('mcp_anywhere.transport.http_server.uvicorn') as mock_uvicorn, \
        ...
        mock_uvicorn.Config.assert_called_once_with(mock_app, host="127.0.0.1", port=9000, log_level="info")
