...
async def test_main_http_command():
    ...
    """
    with patch('mcp_anywhere.__main__.run_http_server', new_callable=AsyncMock) as mock_http_server, \
...
async def test_main_stdio_command():
    ...
    """
    with patch('mcp_anywhere.__main__.run_stdio_server', new_callable=AsyncMock) as mock_stdio_server, \
...
async def test_main_custom_host():
    ...
    """
    with patch('mcp_anywhere.__main__.run_http_server', new_callable=AsyncMock) as mock_http_server, \
         patch('sys.argv', ['mcp-anywhere', 'serve', 'http', '--host', '127.0.0.1', '--port', '9000']):
...
async def test_main_invalid_command():
    ...
    """
    with patch('sys.argv', ['mcp-anywhere', 'invalid-command']), \
         patch('sys.stderr'):  # Suppress stderr output during test
...
async def test_main_missing_subcommand():
    ...
    """
    with patch('sys.argv', ['mcp-anywhere']), \
         patch('sys.stderr'):  # Suppress stderr output during test
...
async def test_main_help_option():
    ...
    """
    with patch('sys.argv', ['mcp-anywhere', '--help']), \
         patch('sys.stdout'):  # Suppress stdout output during test
...
async def test_main_serve_help():
    ...
    """
    with patch('sys.argv', ['mcp-anywhere', 'serve', '--help']), \
         patch('sys.stdout'):  # Suppress stdout output during test
...
def test_main_entry_point():
    ...
    """
    with patch('mcp_anywhere.__main__.main', new_callable=AsyncMock) as mock_main, \
