...
- KEY: DEBUG, DESC: Enable debug logging, REQUIRED: false"""
    ...
    with patch('httpx.AsyncClient') as mock_client_class:
        ...
        mock_pyproject_response.raise_for_status.side_effect = httpx.HTTPStatusError("Not Found", request=Mock(), response=Mock(status_code=404))
        
        mock_client.get.side_effect = [mock_readme_response, mock_package_response, mock_pyproject_response]
...
async def test_fetch_file_success(analyzer):
    ...
    with patch('httpx.AsyncClient') as mock_client_class:
        ...
        mock_response = Mock()
        mock_response.json.return_value = {"content": "dGVzdCBjb250ZW50"}  # "test content" base64 encoded
        mock_response.raise_for_status = Mock()
        mock_client.get.return_value = mock_response
